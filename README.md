...
- Tarkov Dev API는 GraphQL 형식이며, Airflow에서 JSON 데이터를 요청하여 가져온 후 DB에 적재한다.
- 수작업을 최소화하기 위해, Tarkov Dev에서 데이터를 가져온 후 필요한 부분만 수정하는 방식을 채택했다.
- 데이터는 모두 영어이므로, 이름과 같은 고유한 값이 아닌 경우 한글로 자동 변환한다. 예: 회복 아이템의 버프 및 디버프 정의
- Next.js의 SSG 방식 렌더링을 고려하여, 데이터 변경이 바로 반영되지 않는 문제를 해결하기 위해 DAG에서 매일 08:00에 Front를 Reboot한다.
...
Tarkov Dev API에서 반환하는 데이터와 DB 데이터가 일치하지 않는 경우가 발생하여,  
...
이름이 아닌 값들은 한글로 변환해야 하지만, 자동화하기 어려워 현재는 수작업으로 처리.  
