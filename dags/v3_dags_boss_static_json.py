...
def _fetch_targets(postgres_conn_id):
    print("[boss-static] fetch targets from database")
    ...
    print(f"[boss-static] targets fetched: bosses={len(bosses)}")
    return {"bosses": bosses}


def generate_boss_static_json(postgres_conn_id):
    started_at = time.monotonic()
    targets = _fetch_targets(postgres_conn_id)
    root = OUTPUT_DIR / "static" / "boss" / "v3"
    generated_at = pendulum.now("UTC").to_iso8601_string()
    files = {"details": []}

    print(f"[boss-static] output root: {root}")

    for index, normalized_name in enumerate(targets["bosses"], start=1):
        item_started_at = time.monotonic()
        print(
            f"[boss-static] details {index}/{len(targets['bosses'])}: "
            f"{normalized_name}"
        )
        payload = _fetch_json(f"/boss/v3/detail/{quote(normalized_name, safe='')}")
        ...
        print(
            f"[boss-static] details {index}/{len(targets['bosses'])} done: "
...
with DAG(
    dag_id="v3_dags_boss_static_json",
    default_args=default_args,
    start_date=pendulum.datetime(2026, 6, 22, tz="Asia/Seoul"),
    schedule="45 6 * * *",
    tags=["boss", "static-json", "next-public"],
    catchup=False,
) as dag:
    generate_boss_static_json_task = PythonOperator(
        task_id="generate_boss_static_json",
        python_callable=generate_boss_static_json,
