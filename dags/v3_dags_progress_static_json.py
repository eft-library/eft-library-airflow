...
PUBLIC_BASE_PATH = "/static/progress"
REQUEST_TIMEOUT = 60
...
def generate_progress_static_json():
    started_at = time.monotonic()
    root = OUTPUT_DIR / "static" / "progress" / "v3"
    generated_at = pendulum.now("UTC").to_iso8601_string()

    print(f"[progress-static] output root: {root}")

    print("[progress-static] progress-item: start")
    item_started_at = time.monotonic()
    payload = _fetch_json("/progress/v3/progress-item")
    _write_json_atomic(root / "progress-item.json", payload)
    print(
        "[progress-static] progress-item: done "
    ...
    index_payload = {
        ...
        "data": {
            "files": {
                "progress_item": f"{PUBLIC_BASE_PATH}/v3/progress-item.json",
            },
            "counts": {"files": 1},
        },
    }
    ...
    print(
        "Generated progress static JSON: "
        f"output={root}, "
...
with DAG(
    ...
    schedule="15 7 * * *",
    tags=["progress", "static-json", "next-public"],
    catchup=False,
) as dag:
    generate_progress_static_json_task = PythonOperator(
