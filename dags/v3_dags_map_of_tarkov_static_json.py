...
PUBLIC_BASE_PATH = "/static/map-of-tarkov"
REQUEST_TIMEOUT = 60
...
def _fetch_targets(postgres_conn_id):
    print("[map-of-tarkov-static] fetch targets from database")
    ...
    print(f"[map-of-tarkov-static] targets fetched: maps={len(maps)}")
    return {"maps": maps}


def generate_map_of_tarkov_static_json(postgres_conn_id):
    started_at = time.monotonic()
    targets = _fetch_targets(postgres_conn_id)
    root = OUTPUT_DIR / "static" / "map-of-tarkov" / "v3"
    generated_at = pendulum.now("UTC").to_iso8601_string()
    files = {"details": []}

    print(f"[map-of-tarkov-static] output root: {root}")

    for index, normalized_name in enumerate(targets["maps"], start=1):
        item_started_at = time.monotonic()
        print(
            f"[map-of-tarkov-static] details {index}/{len(targets['maps'])}: "
            f"{normalized_name}"
        )
        payload = _fetch_json(
            f"/map-of-tarkov/v3/detail/{quote(normalized_name, safe='')}"
        )
        ...
        print(
            f"[map-of-tarkov-static] details {index}/{len(targets['maps'])} done: "
    ...
    print(
        "Generated map-of-tarkov static JSON: "
...
with DAG(
    dag_id="v3_dags_map_of_tarkov_static_json",
    default_args=default_args,
    start_date=pendulum.datetime(2026, 6, 22, tz="Asia/Seoul"),
    schedule="0 7 * * *",
    tags=["map-of-tarkov", "static-json", "next-public"],
    catchup=False,
) as dag:
    generate_map_of_tarkov_static_json_task = PythonOperator(
        task_id="generate_map_of_tarkov_static_json",
        python_callable=generate_map_of_tarkov_static_json,
