...
PUBLIC_BASE_PATH = "/static/hideout"
REQUEST_TIMEOUT = 60
...
def _fetch_targets(postgres_conn_id):
    print("[hideout-static] fetch targets from database")
    ...
    print(f"[hideout-static] targets fetched: stations={len(stations)}")
    return {"stations": stations}


def generate_hideout_static_json(postgres_conn_id):
    started_at = time.monotonic()
    targets = _fetch_targets(postgres_conn_id)
    root = OUTPUT_DIR / "static" / "hideout" / "v3"
    generated_at = pendulum.now("UTC").to_iso8601_string()
    files = {"details": []}

    print(f"[hideout-static] output root: {root}")

    print("[hideout-static] get-station: start")
    item_started_at = time.monotonic()
    station_payload = _fetch_json("/hideout/v3/get-station")
    _write_json_atomic(root / "get-station.json", station_payload)
    print(
        "[hideout-static] get-station: done "
        f"({time.monotonic() - item_started_at:.2f}s)"
    )

    for index, normalized_name in enumerate(targets["stations"], start=1):
        item_started_at = time.monotonic()
        print(
            f"[hideout-static] details {index}/{len(targets['stations'])}: "
            f"{normalized_name}"
        )
        payload = _fetch_json(f"/hideout/v3/detail/{quote(normalized_name, safe='')}")
        ...
        print(
            f"[hideout-static] details {index}/{len(targets['stations'])} done: "
    ...
    index_payload = {
        ...
        "data": {
            "files": {
                **files,
                "get_station": f"{PUBLIC_BASE_PATH}/v3/get-station.json",
            },
            "counts": {
...
with DAG(
    ...
    schedule="30 7 * * *",
    tags=["hideout", "static-json", "next-public"],
    catchup=False,
) as dag:
    generate_hideout_static_json_task = PythonOperator(
        task_id="generate_hideout_static_json",
